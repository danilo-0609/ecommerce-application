...
internal sealed class ModifyProductColorCommandHandler
    : ICommandRequestHandler<ModifyProductColorCommand, ErrorOr<Guid>>
{
    private readonly IProductRepository _productRepository;

    public ModifyProductColorCommandHandler(IProductRepository productRepository)
    {
        _productRepository = productRepository ?? throw new ArgumentNullException(nameof(productRepository));
    }

    public async Task<ErrorOr<Guid>> Handle(ModifyProductColorCommand command, CancellationToken cancellationToken)
    {
